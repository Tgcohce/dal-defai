...
class ZerePyCLI:
    ...
    def _initialize_commands(self) -> None:
        ...
        self._register_command(
            ...
            Command(
                ...
                description="View the agent's knowledge base content.",
                
                tips=["Use 'view-knowledge' to see all knowledge",
                      "Use 'view-knowledge {topic}' to see specific topic"],
                      
                handler=self.view_knowledge,
        ...
        self._register_command(
            ...
            Command(
                ...
                description="View the agent's system prompt.",
                
                tips=["Shows the full system prompt used by the agent"],
                      
                handler=self.view_system_prompt,
                
                aliases=['prompt', 'system-prompt']
    ...
    def _load_agent_from_file(self, agent_name):
        try: 
            ...
            # Set the global active_agent reference so other modules can access it
            import src.agent
            src.agent.active_agent = self.agent
    ...
    def view_knowledge(self, input_list: List[str]) -> None:
        """View the agent's knowledge base"""
        if self.agent is None:
            logger.info("No agent loaded. Use 'load-agent' first.")
            return
            
        if "knowledge" not in self.agent.connection_manager.connections:
            logger.info("No knowledge base connection found for this agent.")
            return
            
        topic = input_list[1] if len(input_list) > 1 else None
        
        try:
            knowledge_connection = self.agent.connection_manager.connections["knowledge"]
            knowledge = knowledge_connection.get_knowledge(topic)
            
            if not knowledge:
                logger.info(f"No knowledge found{' for topic ' + topic if topic else ''}.")
                return
            ...
            for topic, content in knowledge.items():
                ...
                if isinstance(content, str):
                    ...
                    for subtopic, subcontent in content.items():
                        logger.info(f"  - {subtopic}: {subcontent}")
    ...
    def view_system_prompt(self, input_list: List[str]) -> None:
        """View the agent's system prompt"""
        if self.agent is None:
            logger.info("No agent loaded. Use 'load-agent' first.")
            return
            
        # Force regeneration of system prompt
        self.agent._system_prompt = None
