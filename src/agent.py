...
import src.actions.knowledge_tasks
...
# This will hold a reference to the active agent instance so other modules like Telegram can access it
active_agent = None
...
class ZerePyAgent:
    ...
    def __init__(
        ...
        try:
            ...
            # Track Telegram thread if it exists
    ...
    def _start_telegram_polling(self):
        """Start Telegram polling in a background thread if configured"""
        logger.info("Checking for Telegram connection...")
        
        if "telegram" in self.connection_manager.connections:
            logger.info("Found Telegram connection in connection manager")
            telegram_connection = self.connection_manager.connections["telegram"]
            ...
            if hasattr(self, "telegram_thread") and self.telegram_thread and self.telegram_thread.is_alive():
                logger.info("Telegram polling is already running in a background thread!")
                return
            
            try:
                if telegram_connection.is_configured():
                    logger.info("Telegram connection is configured! Starting polling in background thread...")
                    
                    # Start Telegram polling in a background thread
                    # This is important because run_polling() is blocking
                    self.telegram_thread = threading.Thread(
                        name="TelegramPolling",
                        target=telegram_connection.run,
                        daemon=True  # Allow the main thread to exit even if this thread is still running
                    )
                    ...
                    if self.telegram_thread.is_alive():
                        logger.info("✅ Telegram polling thread started successfully!")
                    else:
                        logger.error("❌ Telegram polling thread failed to start or exited immediately")
                else:
                    logger.warning("Telegram connection exists but is not properly configured. Polling not started.")
            except Exception as e:
                logger.error(f"ERROR starting Telegram polling thread: {e}", exc_info=True)
        else:
            logger.warning("No Telegram connection found in connection manager")
    ...
    def _construct_system_prompt(self) -> str:
        ...
        if self._system_prompt is None:
            ...
            knowledge_connection = self.connection_manager.connections.get("knowledge")
            if knowledge_connection:
                logging.info("\nLoading knowledge base for agent prompt...")
                knowledge = knowledge_connection.get_knowledge()
                if knowledge:
                    # First add a general introduction to the knowledge base
                    prompt_parts.append("\nHere is your knowledge base that contains ACCURATE and UP-TO-DATE information:")
                    
                    # Extract important airdrop information first if it exists
                    airdrop_info = None
                    for topic in ["Airdrop Schedule", "Airdrop Strategy", "Community FAQ"]:
                        if topic in knowledge:
                            if topic == "Airdrop Schedule":
                                airdrop_info = knowledge[topic]
                    
                    # If airdrop information exists, add it with emphasis
                    if airdrop_info:
                        prompt_parts.append("\nIMPORTANT AIRDROP INFORMATION:")
                        prompt_parts.append("You MUST provide this precise information when asked about airdrops:")
                        for subtopic, subcontent in airdrop_info.items():
                            prompt_parts.append(f"- {subtopic}: {subcontent}")
                        logging.info(f"Added emphasized airdrop information")
                    
                    # Now add the rest of the knowledge
                    for topic, content in knowledge.items():
                        # Skip airdrop info since we've already added it
                        if topic == "Airdrop Schedule":
                            continue
                        ...
                        if isinstance(content, str):
                            ...
                            logging.info(f"Added knowledge topic: {topic}")
                        elif isinstance(content, dict):
                            prompt_parts.append(f"\n{topic}:")
                            logging.info(f"Added knowledge topic: {topic} with {len(content)} subtopics")
                            for subtopic, subcontent in content.items():
                                prompt_parts.append(f"- {subtopic}: {subcontent}")
                                logging.info(f"  - Subtopic: {subtopic}")
                        elif isinstance(content, list):
                            prompt_parts.append(f"\n{topic}:")
                            logging.info(f"Added knowledge topic: {topic} with {len(content)} items")
                            for item in content:
                    ...
                    logging.warning("Knowledge connection exists but no knowledge was retrieved!")
            else:
                logging.warning("No knowledge connection found in connection manager!")
